...
# doodle api
# resources: poll and votes
# /poll - get, post DONE
# /poll/:id - get, put delete
# /poll/:id/vote - get, post
# /poll/id/vote/id - get, put, delete
...
class Poll(BaseModel):
    ...
    name: str
    votes: List[Vote]
fake_polls_db = [{"name": "Foo","id": 0}, {"name": "Bar","id": 1}, {"name": "Baz","id": 4}]
highest_poll_id = max(poll['id'] for poll in fake_polls_db)

@app.get("/polls")
async def polls(skip: int = 0, limit: int = 10):
    return fake_polls_db[skip:skip+limit]

@app.get("/polls/{id}")
async def get_poll(id: int):
    for pool in fake_polls_db:
        if pool["id"] == id:
            return pool
...
async def delete_poll(id: int):
    for pool in fake_polls_db:
        if pool["id"] == id:
            fake_polls_db.remove(pool)
            return True
    return JSONResponse(status_code=status.HTTP_404_NOT_FOUND, content=f"Poll does not exist")


@app.post("/polls")
async def create_poll(poll: Poll):
    global highest_poll_id
    highest_poll_id += 1
    poll.id = highest_poll_id
    fake_polls_db.append(poll.dict())
    return poll


# jak zabezpieczyc api
# testy w postmanie / swaggerze
